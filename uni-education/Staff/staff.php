...
<section class="wrapper">
            ...
            <div class="row mt">
                 ...
                 //view data
                  ...
                  $selectString = "SELECT * FROM apply_events WHERE email_status='no'  "; 
                  $comments = mysql_query($selectString);
...
?>                                   
						 ...
						 <br/>
						 <form class="form-horizontal style-form" method="POST" name="send email button">
						 
							<div class="form-group">
                              
                              <div class="col-sm-10">
								  <input type="submit" class="btn btn-round btn-success" name="send-email" value="Send Emails"> 
									<br/>
                              </div>							  
                          </div>
						 <br/>
...
require 'PHPMailer/PHPMailerAutoload.php';
     $selectString = "SELECT * FROM apply_events where email_status='no' LIMIT 2"; 
                  $comments = mysql_query($selectString);
...
$mail->Username = 'sachinthasam91@gmail.com';          // SMTP username
$mail->Password = 's0418h0913'; // SMTP password
$mail->SMTPSecure = 'tls';                  // Enable TLS encryption, `ssl` also accepted
$mail->Port = 587;                          // TCP port to connect to

$mail->setFrom('sachinthasam91@gmail.com', 'CodexWorld');
$mail->addReplyTo($row['semail'], 'CodexWorld');
$mail->addAddress($row['semail']);   // Add a recipient
//$mail->addCC('cc@example.com');
//s$mail->addBCC('bcc@example.com');
...
$selectString2 = "UPDATE apply_events SET email_status='yes' LIMIT 2"; 
                  $comments = mysql_query($selectString2);
						 }
   ...
   $.ajax({
      ...
      success: function(data, status) {
        ...
        if(data == "0") {
          ...
          $.notify("You have :"+data+" new events", "info");
         
         

              $.ajax({
              url: 'changeNotifyStatus.php',
              type: 'post',
              data: {'action': 'change'},
              success: function(data, status) {
                console.log(data);
